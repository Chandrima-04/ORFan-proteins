...
.PHONY: all sam tsv html

all: tsv

sam: sam/$(out_prefix).sam
tsv: tsv/$(out_prefix).tsv
html: html/$(out_prefix).html
...
blast_tsv_cols := qseqid sseqid pident qcovs qcovhsp score length mismatch gapopen qlen qstart qend slen sstart send 
...
sam/%.sam: asn/%.asn
	mkdir -p $(dir $@)
	blast_formatter -archive $< -outfmt 17 -out $@
